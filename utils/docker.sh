...
start_docker () {
    echo "正在启动 Docker 容器 . . ."
    docker run -dit --restart=always --name="$container_name" --network=host --hostname="$container_name" teampgm/pagermaid_pyro <&1
    echo
    echo "开始配置参数 . . ."
        ...
        [yY][eE][sS] | [yY])
            ...
            if [ -d $data_path ]; then
                ...
                if docker inspect $container_name &>/dev/null; then
                    ...
                    docker run -dit -e PUID=$PUID -e PGID=$PGID -v $data_path/workdir:/pagermaid/workdir --network=host --restart=always --name="$container_name" --hostname="$container_name" teampgm/pagermaid_pyro <&1
                    echo
                    echo "数据持久化操作完成。"
                    echo 
                    shon_online
